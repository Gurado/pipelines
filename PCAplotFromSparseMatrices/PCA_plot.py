#!/bin/python
######################################
# calculate using sklearn RandomizedPCA
#
...
def readFileIntoSparseMatrix(fragmentsLookupTable, fragmentsCount):
	...
	for contactCountsFile in args.contactsCountMatrices:
		...
		for line in infile:
			ch1,mid1,ch2,mid2,contactCounts,pvalue,qvalue=line.split()

			if (float(qvalue) > args.treshold):
				continue

			contactCounts=int(contactCounts)
			...
			if (fragment1 < fragment2):
				A[c, fragmentsLookupTable[fragment1] * fragmentsCount + fragmentsLookupTable[fragment2]] = contactCounts
			else:
				A[c, fragmentsLookupTable[fragment2] * fragmentsCount +  fragmentsLookupTable[fragment1]] = contactCounts
...
def plotVariance(S, N, explainedRatio, totalExplained):
	...
	f.write(" + xlab('%.1f%% variance explained')" % (explainedRatio[0]*100))
	f.write(" + ylab('%.1f%% variance explained')" % (explainedRatio[1]*100))
	...
	f.write(" + geom_point(aes(%s), alpha = 0.5, size=5)" %(','.join(s)))
	f.write(" + xlab('%.1f%% variance explained')" % (explainedRatio[1]*100))
	f.write(" + ylab('%.1f%% variance explained')" % (explainedRatio[2]*100))
	f.write(" + geom_text(aes(label=experiment, color=experiment), size=3, vjust=3)")
	...
	f.write("pdf('"+args.imagename+".pdf')\n")
	f.write("g1\n")
	f.write("g2\n")
	f.write("dev.off()\n")
...
def main():
	...
	'''
	global args
	parser = argparse.ArgumentParser(description='Reads a list of significant interaction sparse matrix files and performs PCA')
	...
	parser.add_argument("-t", "--threshold", type=float, dest="treshold", default=0.01, 
